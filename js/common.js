	...
	Kimkra.videoview = function(data) {
		...
		poster.appendChild(_createElement("img", {
			...
			tap: function() {
				...
				Kimkra.setVideoCache(Kimkra.tempVideos.temp.type, Kimkra.tempVideos.list);
	...
	var Autoslider = mui.autoslider = mui.Slider.extend({
		...
		_triggerSlide: function(e) {
			...
			if(!self.lastPage || (self.currentPage.element.id != self.lastPage.element.id)) {
				...
				self.currentPage.element.querySelector("iframe") && self.currentPage.element.querySelector("iframe").contentWindow.dispatchEvent(new CustomEvent("showVideo"));
			}
			self._initTimer();
		},
			...
			if(Kimkra.dispatchSlider) {
				...
				if(direction === 'left' || direction === 'right') {
					if(Kimkra.dispatchTouch) {
						...
						//拖拽期间隐藏视频 防止x5抽风
						e.target.querySelector("iframe") && e.target.querySelector("iframe").contentWindow.dispatchEvent(new CustomEvent("hideVideo"));
					}
					//拖拽期间取消定时
